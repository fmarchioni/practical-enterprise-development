...
/subsystem=elytron/filesystem-realm=tlsFsRealm:add-identity(identity=ejbuser)

# Add the password for the example identity
/subsystem=elytron/filesystem-realm=tlsFsRealm:set-password(identity=ejbuser, clear={password=password123})

# Add the guest role for the example identity
/subsystem=elytron/filesystem-realm=tlsFsRealm:add-identity-attribute(identity=ejbuser, name=Roles, value=[employee])
...
/subsystem=elytron/key-store=tlsTrustStore:import-certificate(alias=client,path=$CURR_DIR/tlsClient.cer,credential-reference={clear-text=serverTrustSecret},trust-cacerts=true,validate=false)
...
keytool -importcert -keystore $CURR_DIR/tlsClient.truststore -storepass clientTrustSecret -alias localhost -trustcacerts -file $WILDFLY_HOME/standalone/configuration/tlsServer.cer -noprompt

$WILDFLY_HOME/bin/elytron-tool.sh credential-store --create --location "tlsCredStore.cs" --password clientStorePassword
$WILDFLY_HOME/bin/elytron-tool.sh credential-store --location "tlsCredStore.cs" --password clientStorePassword --add ejbuser --secret password123
